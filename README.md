...
Обратное сопоставление — из каталога в источник — строгое. Поиск источника
нечёткий: по запросу «Атака титанов» AniLibria отвечает «Не издевайся,
Нагаторо: Вторая атака», потому что там есть слово «атака». Поэтому вместе
с русским названием передаётся оригинальное, названия нормализуются
и сравниваются по словам, а кандидат ниже порога отбрасывается целиком —
лучше сказать «не нашлось», чем открыть чужой тайтл.

Остальные источники опрашиваются не при каждом открытии, а по делу: если
выбранный ничего не нашёл, если у него нет серий или если он не отдал видео
при запуске. Открытие тайтла стоит двух запросов вместо четырёх десятков.
Кнопка «Поискать в других источниках» рядом с выбором делает то же вручную.
...
Окно запоминает размер и положение между запусками. Обложки, которых нет
или которые источник отдал битой ссылкой, показываются заглушкой с инициалами
названия, а не иконкой сломанной картинки. Пустая библиотека, история
и загрузки не тупик: там сразу лежат кнопки «Найти аниме» и «Открыть каталог».
...
Проверено на живом API: у «Фрирен» тридцать восемь плееров на серию, и все
тридцать восемь без токена — Kodik, у которого потолок 720p. Поле `video`
с массивом качеств приходит только с заголовком `Authorization`. То есть
AnimeLib без токена строго хуже Yummy Anime, и приложение это учитывает:
пока токена нет, AnimeLib уступает место в приоритете и не выбирается
источником по умолчанию.

Ещё одна особенность каталога: поиск ранжирует плохо. По запросу «Атака
титанов» он отдаёт три нарезки-обзора на десять, пятнадцать и двадцать минут
с нулём серий — самого тайтла на AnimeLib просто нет, его лицензировали
и убрали. Поэтому результаты пересортировываются по совпадению названия,
числу оценок и рейтингу, рекапы и промо уезжают вниз, а источник, у которого
для тайтла ноль серий, уступает тому, у кого они есть.
